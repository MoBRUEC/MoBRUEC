...
- **Diagrams on pages:** use ```` ```mermaid ```` fences for rendered visual diagrams (mermaid.js is in the layout, runs on `.language-mermaid`; see jArchi-Build for the pattern). Keep PlantUML in ```` ```plantuml ```` fences only where the code itself is the deliverable (copy-paste templates/examples). After adding mermaid, Playwright-verify the SVG actually renders (`.language-mermaid svg` present, zero console errors) — mermaid fails client-side and silently.
